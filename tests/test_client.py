...
#
# The SDK runs synchronous tool handlers on worker threads
# (anyio.to_thread.run_sync), so several tool calls share one client
# concurrently -- where 1.x serialized them on the event loop. Auth is a
# read-modify-write over (_user_id, _token, _timezone) plus a session-file
# write, so an unguarded burst produces one login per thread against a
# rate-limited endpoint (issue #3) and interleaved session.json writes.
# ---------------------------------------------------------------------------
...
def test_concurrent_expired_token_logs_in_once(tmp_path):
    ...
    Each request re-authenticates against the token *it* sent, so threads that
    were rejected for the same expired token do not each burn a login -- and
    none of them discards the fresh token a peer just won.
    """
    client = CronometerClient(session_path=tmp_path / "session.json")
    client._user_id = 123
    client._token = "STALE_TOKEN"
