...
def test_server_reports_a_version():
    ...
    SDK 2.x reports "" for a server constructed without `version=` (1.x
    substituted the SDK's own version), so an omitted argument is invisible
...
def test_tool_annotations_survive_schema_coercion():
    ...
    They are declared in camelCase (`readOnlyHint`) and SDK 2.x renamed the
    Python attributes to snake_case, keeping camelCase only as an input alias.
    A silent coercion failure would strip the hints clients use to decide what
...
def test_concurrent_get_client_builds_one_instance(monkeypatch):
    ...
    Two clients would mean two logins against a rate-limited endpoint
    (issue #3) and two writers for one session file.
    """
    from cronometer_api_mcp import server
